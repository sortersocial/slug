        ...
        div class="ont-item-pin-zone" data-garden-room=(room_wire.as_str()) {
            ...
            @if let Some(pi) = pinned_item {
                @if pi == &current {
                    ...
                    a class="ont-vote-compare-btn" href=(vote_compare_href(nav, pi, &current, None, None, None)) title="Compare and vote" {
        ...
        @if let Some(pi) = pinned_item {
            span class="ont-garden-child-actions" data-garden-room=(nav.room_wire.as_str()) {
                @if pi == row_item {
                    ...
                    a class="ont-garden-vote-ico" href=(vote_compare_href(nav, pi, row_item, None, None, None)) title=(tip) aria-label=(aria) {
