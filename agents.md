...
- Forum and garden item bodies use **`~/…` linkification** (`linkify_slugs_with_prefix` in `server/src/html/mod.rs`). Tilde refs emit **leaf hrefs** (`~/x/luke` → `/~/luke`). Nested *written* paths keep their display text (`~/x/luke`) so path sugar stays visible in threads; already-leaf refs (`~luke`, `~/luke`) and `https://slug.social/~/…` URL spellings still show as the leaf. When **`item_bodies`** is in scope, matching ontology links get a native **`title`** tooltip with a **truncated body preview** (hover in the browser).
