...
- **Check / `check_sorter` diagnostics:** Parse errors include a 1-based **`line N:`** prefix. A dotted tilde token (`~mcdonalds.com`) fails on that line (`invalid character '.'`; slugs are `[a-z0-9_-]`), not on a later valid claim. A URL-shaped line without `http(s)://` (`mcdonalds.com { body }`) stays **prose** and adds a warning. `https://… { body }` remains a URL item (not prose). **`CheckOk.warnings`** also notes when a document defined items but includes no votes (ranking preview empty).
