...
| **Aspect groups / prompts** | **Derived** | `ContentState.aspect_groups` keyed `(parent, slug)` and `aspect_prompts` keyed by slug (per room `ContentState`). Rebuilt from DSL on ingest replay; never persisted. |
