...
- **Muse connector (`/muse/v1`)** (`server/src/muse/`): Consumer Muse (muse.ai Connector Platform) does **not** speak MCP. It builds a custom connector from a **public OpenAPI 3.0.3 spec** plus a `slug_…` bearer in Meta's Secure Credentials Store. Same tool catalog and `call_named_tool` / `dispatch_rpc` path as `POST /mcp`. `GET /muse/v1/status` is the unauthenticated liveness check (`{"ok":true,"status":"ok"}`, same as MCP `health` / `GET /healthz`). `GET /muse/v1/whoami` is the credential check. `GET /muse/v1/openapi.json` and `GET /muse/v1/docs.md` need no login (Muse fetches them from its VM). `GET /muse` is the shareable HTML listing (home CTA **connect Muse**); `GET /muse.md` is the same brief as `/muse/v1/docs.md`. `POST /muse/v1/:tool` JSON body = MCP `inputSchema`; success returns MCP `structuredContent`; auth failures are HTTP 401 with `WWW-Authenticate`. Do not add a parallel write path. Muse Code (the terminal agent) still uses `POST /mcp`. Directory submission is https://muse.ai/platform (Meta login required) pointing at `/muse` + the public spec.
